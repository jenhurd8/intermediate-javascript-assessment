...
charger.drive(this.gasRemaining);
mustang.drive(this.gasRemaining);
mustang.drive(this.gasRemaining);
...
String.prototype.grammarPolice = function() {
  var newString = this.split(" ");
  var newArray = [];

  //newString.replace(:%s/.*/\L&/)
  // for (var i = 0; i < newString.length; i++) {
  // var newWord = "";
  //   for(var j = 0; j < newString[i].length; j++){
  //     newWord = "";
  //     if(typeof(newString[i][0])===String)){
  //       newWord = newString[i][0].toUpperCase();
  //     }
  //     if(typeof(newString[i][j]===String)){
  //       newWord = newWord + newString[i][j].toLowerCase();
  //     }
...
function promiseCatcher(a) {
  ...
  if (a.isFulfilled()) return (theAnswer = a);
}
